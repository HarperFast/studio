...
The wide hero artwork spans both columns beneath the form card. A separately aligned, theme-specific globe extension sits underneath the original image, preserving the platform and particle coordinates while adding surface below and to both sides. Keep its max-width unrestricted; the footer’s bottom margin reserves its overflow without moving the form or platform. Keep the headline and feature copy as HTML. `HeroParticles` uses the artwork's 2054 × 766 coordinate system, with separate paths for each theme; replacing either image requires retracing that theme's purple arcs. Reduced-motion mode hides particles and disables decorative animation. Linux visual baselines use reduced motion so animated decoration does not introduce pixel-diff noise.
