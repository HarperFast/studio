...
- **Two sources.** Studio's own Harper `restart` runs mark and release their targets (`markRestarting`, released in `finally`) — central manager never hears about these. Container ops are central manager's: every `getClusterInfo` fetch mirrors its `RESTARTING`/`STARTING` statuses into the tracker. A new flow that takes an instance down must use one of the two, or its failures toast like before.
- **Requests are held, not failed.** Every `getInstanceClient` client waits while its entity is `down` ([`restartGate.ts`](src/lib/restart/restartGate.ts)). Requests that drive or watch a restart must pass `skipRestartGate: true`, or they wait on themselves.
