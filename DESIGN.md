...
- **Forms**: react-hook-form with a zod `zodResolver`. A form whose submit button waits on `formState.isValid` needs `mode: 'onChange'`: the default `onSubmit` mode keeps `isValid` current but shows errors only after a submit, which the disabled button never allows, so the user is blocked with no reason given (HarperFast/studio#1550). A refine that compares two fields reports on one of them; give the other field `rules={{ deps: [...] }}` so the error also follows edits to it.
