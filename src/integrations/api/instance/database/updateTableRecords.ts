...
export function describeIncompleteUpdate(
	...
	if (isMalformedHashes(data?.update_hashes) || isMalformedHashes(data?.skipped_hashes)) {
		return "Harper's response didn't report which records it wrote, so the change may not have been saved.";
	}
	...
	return `Harper updated ${written} of ${recordCount} records${
