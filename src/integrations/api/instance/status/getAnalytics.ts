...
// (typed as AnalyticsDataPoint) so each spec's own `timestamp:` field decides
// which column drives the x-axis. Query key is instance-scoped so cross-instance
// studio tabs don't collide in React Query's cache.
...
interface GetRawAnalyticsRequest {
